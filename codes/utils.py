...
def run_all(n, m, mu, cond_n, max_iter, x_0, coef_step, c=1., exp=.55, mean=0., std_dev=1., 
            inner_iters=1, numb_exp=1, numb_pb=1, save=False, root="", ols=True, plot=False):
    
    sigma = mu
    for pb in range(numb_pb):
        np.random.seed(pb)
        A, b, x_true, _ = make_data_reg(n, m, cond_numb_A=cond_n, noise_scale=0.2, 
                                        mu=np.sqrt(n*mu), 
        ...
        x_opt = grad_desc(A, b, L_grad, max_iter*10000)
        f_star = least_square_loss(A, b, x_opt)
        
        L = np.max((A ** 2).sum(axis=1))
        # print(norm(A,ord=-2)**2 / n, norm(A,ord=2)**2 / norm(A,ord=-2)**2, L, L_grad)
        
        # m_fix = int((8*(2*L-mu_)) / mu_) + 1
        # delta = mu_**2 * m_fix**2 - 8 * (2*L - mu_)*mu_*m_fix
        # dem = 4*(2*L - mu_)*mu_*m_fix
        # step_svrp = (mu_*m_fix + np.sqrt(delta)) / dem
        # inner_iters = m_fix

        # m_fix = int((8*(2*L-mu_)) / mu_) + 1
        # delta = mu_**2 * m_fix**2 - 8 * (2*L - mu_)*mu_*m_fix
        # dem = 4*(2*L - mu_)*mu_*m_fix
        # step_svrp = .5 * ( ((mu_*m_fix + np.sqrt(delta)) / dem) 
        #           + ((mu_*m_fix + np.sqrt(delta)) / dem) )
        # inner_iters = m_fix
        
        step_svrp = 1. / (coef_step[0]*L)
        m = 1. /(mu_*step_svrp*(1 - 2*step_svrp*(2*L-mu_)))
        inner_iters = int(m) + 1
        
        # print(inner_iters, m_fix)
        
        step_sapa = 1. / (coef_step[1]*L)
 
        if inner_iters != 0:
            total_elt_op = max_iter * (n + inner_iters + 1) - n
            total_svrg = max_iter # + 1
            total_sppa = max_iter * (n + inner_iters + 1)
        dict_data = {}

        fx_2 = np.zeros(total_sppa)

        fx_8 = np.zeros(total_elt_op)

        # fx_3 = np.zeros(max_iter * inner_iters)
        fx_3 = np.zeros(max_iter)

        if numb_exp >= 1:

            fx_2_ = np.zeros(total_sppa)

            fx_8_ = np.zeros(total_elt_op)

            # fx_3_ = np.zeros(max_iter * inner_iters)
            fx_3_ = np.zeros(max_iter)

            for i in range(numb_exp):
                
                print(f"cond {cond_n}, sigma {sigma}, experience {i}")

                # tmp = sapa_ols(A, b, step_sapa, x_0, total_elt_op, rnd_seed=None)
                fx_8_ += sapa_ols(A, b, step_sapa, x_0, total_elt_op, rnd_seed=None)

                # tmp = stoch_prox_least_square(A, b, x_0, total_sppa, c=c, exp=exp)
                fx_2_ += stoch_prox_least_square(A, b, x_0, total_sppa, c=c, exp=exp)
                
                # s = np.random.choice(np.arange(inner_iters), max_iter)
                fx_3_ += svrp_ols(A, b, step_svrp, x_0, total_svrg, inner_iter=inner_iters, 
                                  rnd_seed=None)[0]
        ...
        f_star = min((np.min(fx_8), np.min(fx_3),
                         np.min(fx_2), f_star))
    dict_data  = {
                        "SAPA": (fx_8 - f_star) / numb_pb,
                        "SPPA": (fx_2 - f_star) / numb_pb,
                        "SVRP": (fx_3 - f_star) / numb_pb,
                        }
    
    except_ = []
    plot(dict_data, mu_, sigma, cond_n, max_iter, total_elt_op, max_bound=0, excepted=except_, 
         normalized=True,
         step_rule=f"mu_{mu}_cond_{int(cond_n)}_step_{coef_step}L_max_{max_iter}_in_{inner_iters}.png",
         root="images/timing/test_2/", save=save, plot=plot)
